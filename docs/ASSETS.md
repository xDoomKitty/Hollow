...
All game-specific graphics are original code-native art in `game/scripts/map_view.gd`: underground floors/walls, transit tracks, containers, camp structures, colonist coats/packs/lamps, creatures, stairs, fog and map overlays. Wayfarer adds original wagon/roadstead, screw-jack, paired-lane and shared-load ledger symbols. `game/assets/icon.svg` is an original vector icon. No assets were extracted from RimWorld, Project Zomboid or another game.
